...
\section {Функция AckleyII}

\subsection {Описание функции}

\begin{tabularwide}
\textbf{Идентификатор:} & MHL\_TestFunction\_AckleyII. \\
\textbf{Наименование:} & Функция AckleyII. \\
\textbf{Тип:} & Задача вещественной оптимизации. \\
\end{tabularwide}

\textbf{Формула} (целевая функция):
\begin{equation}
\label{TestFunctions:eq:MHL_TestFunction_AckleyII}
f\left( \bar{x}\right) = 20 + e - 20e^{-0.2\sqrt{\frac{1}{n}\sum_{i=1}^{n}\bar{x}_i^2}}-e^{\frac{1}{n}\prod_{i=1}^{n}cos\left( 2\pi\cdot\bar{x}_i\right) }, \text{ где}
\end{equation}
\indent $\bar{x}\in X$, $\bar{x}_j\in \left[ Left_j; Right_j\right] $, $Left_j=-5$, $Right_j=5$, $j=\overline{1,n}$.
...
\textbf{Решаемая задача оптимизации:} & $\bar{x}_{min}= \arg \min_{\bar{x}\in X} f\left( \bar{x}\right)$.   \\
\textbf{Точка минимума:} & $\bar{x}_{min}={\left( 0,0,\ldots,0\right)}^\mathrm{T} $, то есть $\left(\bar{x}_{min} \right)_j=0$ ($j=\overline{1,n}$).    \\
\textbf{Минимум функции:} & $f\left(\bar{x}_{min} \right) =1.069560557758917$.   \\
\textbf{График:} & Рисунок \ref{TestFunctions:img:MHL_TestFunction_AckleyIIe} нас \pageref{TestFunctions:img:MHL_TestFunction_AckleyIIe} стр.   \\
\end{tabularwide}

\begin{figure} [h] 
  \center
  \includegraphics [scale=0.5] {MHL_TestFunction_AckleyII}
  \caption{Функция AckleyII} 
  \label{TestFunctions:img:MHL_TestFunction_AckleyIIe}  
\end{figure}
...
\begin{lstlisting}[caption=Код функции MHL\_TestFunction\_AckleyII]
...
Данная функция является авторской модификацией, поэтому данный документ является основным ее описанием.
